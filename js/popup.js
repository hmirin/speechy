document.addEventListener("DOMContentLoaded", function () {
  ...
  document.getElementById("help_link").addEventListener("click", function () {
    chrome.tabs.create({
      url: "https://hmirin.github.io/speechy/installed#usage",
    });
  });
...
function saveOptions() {
  chrome.storage.sync.set({
    api_provider: document.querySelector('input[name="api_provider"]:checked')
      .value,
...
function switchApiOptions(api_provider) {
  ["google", "openai"].forEach((provider) => {
    document.querySelectorAll(`.${provider}`).forEach((el) => {
      el.style.display =
        provider === api_provider.toLowerCase() ? "block" : "none";
    });
  });
}
